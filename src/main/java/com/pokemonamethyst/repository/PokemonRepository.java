...
public interface PokemonRepository extends JpaRepository<Pokemon, String> {
    ...
    @Query("SELECT p FROM Pokemon p WHERE p.perfil.id = :perfilId AND p.ordemTime IS NOT NULL AND p.ordemTime BETWEEN 1 AND 6 ORDER BY p.ordemTime")
    List<Pokemon> findTimePrincipalByPerfilId(String perfilId);

    @Query("SELECT p FROM Pokemon p WHERE p.perfil.id = :perfilId AND (p.ordemTime IS NULL OR p.ordemTime = 0)")
